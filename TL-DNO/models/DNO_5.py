...
class DNO(nn.Module):
    ...
    def __init__(self, num_channels, width, initial_step, pad=2, factor=1, pad_both=False):
        ...
        self.conv0 = OperatorBlock_3D(self.width, factor * self.width, 48, 48, 10, 20, 20, 8, Normalize=True)

        self.conv1 = OperatorBlock_3D(factor * self.width, 2 * factor * self.width, 32, 32, 10, 14, 14, 8)

        self.conv2 = OperatorBlock_3D(2*factor*self.width, 4*factor*self.width, 16, 16, 12,  10,10,8)

        self.conv3 = OperatorBlock_3D(4*factor*self.width, 4*factor*self.width, 16, 16, 12,  10,10,8, Normalize = True)

        self.conv6 = OperatorBlock_3D(8*factor*self.width, 2*factor*self.width, 32, 32, 18,  10,10,8)

        self.conv7 = OperatorBlock_3D(4 * factor * self.width, factor * self.width, 48, 48, 20, 14, 14, 8,
                                      Normalize=True)
    ...
    def forward(self, x):
        ...
        x_c3 = torch.cat([x_c3, torch.nn.functional.interpolate(x_c2, size=(x_c3.shape[2], x_c3.shape[3], x_c3.shape[4]),
        ...
        x_c8 = torch.cat([x_c8,
                          torch.nn.functional.interpolate(x_fc0, size=(x_c8.shape[2], x_c8.shape[3], x_c8.shape[4]),
                                                          mode='trilinear', align_corners=True)], dim=1)
        # print('x_c8', x_c8.shape)
