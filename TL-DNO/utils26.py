...
class SinkhornDistance(torch.nn.Module):
    ...
    def _cost_matrix(x, y, p=2):
        ...
        # print(f"Before cost matrix calculation: {torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB")
        C = torch.sum((torch.abs(x_col - y_lin)) ** p, -1)
        # torch.cuda.empty_cache()

        # print(f"After cost matrix calculation: {torch.cuda.memory_allocated() / 1024 ** 3:.2f} GB")
        return C
    # def _cost_matrix(x, y, p=2, chunk_size=1024):
    #     """Returns the matrix of $|x_i-y_j|^p$ with memory-efficient batching."""
    #     N, P1, D = x.shape
    #     _, P2, _ = y.shape
    #     C = torch.zeros((N, P1, P2), device=x.device)
    #     for i in range(0, P1, chunk_size):
    #         x_chunk = x[:, i:i + chunk_size].unsqueeze(-2)
    #         diff = torch.abs(x_chunk - y.unsqueeze(-3)) ** p
    #         C[:, i:i + chunk_size] = torch.sum(diff, dim=-1)
    #
    #     return C
...
def bce_adv_DS(inputs1, inputs2):
    truth_tensor = torch.FloatTensor(inputs1.size())
...
def bce_adv_DT(inputs1, inputs2):
    truth_tensor = torch.FloatTensor(inputs1.size())
